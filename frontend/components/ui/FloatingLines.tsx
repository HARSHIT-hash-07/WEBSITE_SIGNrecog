...
float wave(vec2 uv, float offset, vec2 screenUv, vec2 mouseUv, bool shouldBend) {
  ...
  float m = uv.y - y;
  // Subtle thickness for dark background
  return 0.015 / max(abs(m) + 0.01, 1e-3) + 0.005;
}
...
export default function FloatingLines({
  ...
  useEffect(() => {
    ...
    const bgColor = hexToVec3(backgroundColor);
    ...
    const uniforms = {
      ...
      brightnessMultiplier: { value: brightnessMultiplier },
    };
    ...
    // Use window-level event for better mouse tracking
    ...
    if (interactive) {
      // Use window-level event for reliable mouse tracking
